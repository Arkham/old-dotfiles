...
\begin{abstract}
  ...
  technologies, like MPLS routers, Ethernet switches and optical
  cross-connects.
...
\begin{abstract}\thispagestyle{empty}
  ...
  Italian friends and my family for their constant support.
\end{abstract}
...
\nomenclature{ASE}{Amplifier Spontaneous Emission}
...
\nomenclature{OSNR}{Optical Signal-to-Noise Ratio}
...
\nomenclature{PMD}{Polarization Mode Dispersion}
...
\nomenclature{VTY}{Virtual TeletYpe}
...
information concerning the state of the data plane, \textit{LSP
  Notify} messages are used. In Table~\ref{tab:rsvp_messages} we can
...
widely used to solve many shortest-path computation problems (only in
the case of single-pair problems, the performance of Dijkstra is
better).
...
\footnote{\url{http://dragon.maxgigapop.net/twiki/bin/view/DRAGON/}}
...
\begin{figure}[!htbp]
  \begin{center}
    ...
    \caption[The Dragon model]{The various DRAGON elements and the
...
\subsection{Resource Computation Element}
...
\begin{figure}[!htbp]
  \begin{center}
    \includegraphics[width=0.9\textwidth]{img/rce_model}
    \caption[The RCE model]{The various RCE elements and the
      interactions between them.}
    \label{fig:rce_model}
  \end{center}
\end{figure}
...
a specific LSP sent back to the client; this LSP has been computed
...
\bibliography{bibliography} \bibliographystyle{is-alpha}
